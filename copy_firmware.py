...
def copy_firmware(source, target, env):
    ...
    # Read FW_VERSION from robot_tenis_v7.cpp
    version = "unknown"
    ...
    try:
        with open(main_cpp, "r", encoding="utf-8") as f:
            ...
            if m:
                ...
                # Fallback: try old format #define FW_VERSION
                m = re.search(r'#define\s+FW_VERSION\s+"([^"]+)"', content)
                if m:
                    version = m.group(1)
                    print(f"[POST-BUILD] Found FW_VERSION (legacy #define): {version}")
                else:
                    print(f"[POST-BUILD] WARNING: Could not find FW_VERSION in {main_cpp}")
    except Exception as e:
        print(f"[POST-BUILD] ERROR reading {main_cpp}: {e}")
